...
static uint8_t mspm0_i2c_timed_out(uint32_t start_ms)
{
    return ((uint32_t)(Board_GetMillis() - start_ms) >=
            I2C_TIMEOUT_MS) ? 1U : 0U;
}
                     ...
                     unsigned char const *data)
{
    ...
    while (!(DL_I2C_getControllerStatus(I2C_0_INST) &
        ...
        if (mspm0_i2c_timed_out((uint32_t)start) != 0U) {
                    ...
                    unsigned char *data)
{
    ...
    do {
        ...
        I2C_0_INST, DL_I2C_INTERRUPT_CONTROLLER_RX_DONE));
