...
/*
 * Software-SPI timing guard.
 *
 * CPUCLK is 80 MHz in the current SysConfig. Direct back-to-back GPIO writes
 * made SCL timing depend on compiler optimisation and became marginal when
 * the packaged mode menu sent several complete frames consecutively.
 */
static void OLED_SpiEdgeDelay(void)
{
    delay_cycles(8U);
}
...
void OLED_ShowColorResult(const uint8_t *color_text)
{
    if (color_text == NULL) {
        return;
    }
...
void OLED_Init(void)
{	
	/* Establish a defined idle state before resetting the panel. */
	OLED_SCLK_Set();
	OLED_SDIN_Set();
	OLED_RS_Set();
	OLED_RST_Set();
